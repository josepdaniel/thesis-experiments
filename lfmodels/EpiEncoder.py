...
class RelativeEpiEncoder(nn.Module):
    ...
    def encode(self, formatted, stacked):
    ...
    def forward(self, tgt_lf_formatted, tgt_lf_stacked, ref_lfs_formatted, ref_lfs_stacked):
        ...
        Returns:
            - tgt_encoded: the encoded tgt light field concatenated with the stacked image-grid   [B, N+16, H, W]
            - ref_encoded: the encoded ref light fields as a list
        """
        
        tgt = self.encode(tgt_lf_formatted, tgt_lf_stacked)
        ref = [self.encode(formatted, stacked) for formatted, stacked in zip(ref_lfs_formatted, ref_lfs_stacked)]
...
class EpiEncoder(nn.Module):
    ...
    def forward(self, lf_formatted, lf_stacked):
        ...
        Arguments:
            ...
            - lf_stacked: the grid of images stacked on the colour-channel   [B, N, H, W]
        Returns:
            - encoding: the encoded light field concatenated with the stacked image-grid   [B, N+16, H, W]
        """
