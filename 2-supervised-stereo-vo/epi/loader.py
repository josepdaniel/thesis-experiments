    ...
    def __call__(self, datapoint):
        ...
        for i, im in enumerate(images):
            for c in range(im.shape[2]):
                im[:,:,c] = cv2.flip(im[:,:,c], 1)
            images[i] = im
        datapoint["images"] = images
        

        poses = datapoint["poses"]
        poses[:, 2] = poses[:, 2] * -1      # Correct the rotation about z axis
        poses[:, 3] = poses[:, 3] * -1      # Correct the translation about x axis
        datapoint["poses"] = poses          # Put the data back in
